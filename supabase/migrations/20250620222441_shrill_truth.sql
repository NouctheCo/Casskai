/*
  ...
  1. New Tables
    - `company_tax_rates`
      ...
      - `type` (text, tax type like TVA, IS, etc.)
    ...
    - `company_tax_declarations`
      - `id` (uuid, primary key)
      - `company_id` (uuid, foreign key to companies)
      - `type` (text, declaration type like TVA, IS, etc.)
  ...
  2. Security
    ...
    - Add policies for authenticated users to manage their company's tax data

  3. Indexes
    - Add indexes for frequently queried columns
    - Add composite indexes for company_id + other fields
*/
...
CREATE TABLE IF NOT EXISTS company_tax_payments (
  ...
  payment_method text NOT NULL CHECK (payment_method IN ('bank_transfer', 'check', 'cash', 'card', 'online')),
  reference text,
...
CREATE TABLE IF NOT EXISTS company_tax_documents (
  ...
  name text NOT NULL,
  type text NOT NULL CHECK (type IN ('declaration', 'receipt', 'certificate', 'supporting_document', 'other')),
...
CREATE INDEX IF NOT EXISTS idx_company_tax_payments_date ON company_tax_payments(company_id, payment_date DESC);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_company_tax_rates_updated_at'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_company_tax_declarations_updated_at'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_company_tax_payments_updated_at'
  ) THEN
