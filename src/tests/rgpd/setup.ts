...
if (!TEST_SUPABASE_URL || !TEST_SUPABASE_ANON_KEY) {
  logger.warn('Setup', '⚠️ Variables Supabase test non configurées. Tests RGPD seront skippés.');
}
// Setup avant tous les tests
beforeAll(async () => {
  logger.debug('Setup', '🧪 Setup tests RGPD...');
  ...
  try {
    ...
    if (response.ok) {
      logger.debug('Setup', '✅ Connexion Supabase test OK');
    } else {
      logger.warn('Setup', '⚠️ Connexion Supabase test échouée:', response.status);
    }
  } catch (error) {
    logger.warn('Setup', '⚠️ Impossible de se connecter à Supabase test:', error);
  }
});
// Cleanup après tous les tests
afterAll(async () => {
  logger.debug('Setup', '🧹 Cleanup tests RGPD...');
