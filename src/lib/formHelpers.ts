...
export function useInputHelpers<T extends FieldValues = FieldValues, K extends Path<T> = Path<T>>(
  ...
  const handleChange = useCallback((newValue: unknown) => {
    ...
    const parsedValue = options?.parser ? options.parser(newValue as string) : newValue;
    ...
    form.setValue(fieldName, transformedValue as any);
...
export function useCurrencyInput<T extends FieldValues = FieldValues>(
  ...
  return useInputHelpers(fieldName, {
    formatter: (value: unknown) => Formatters.currency(typeof value === 'number' ? value : 0, currency, locale),
...
export function useDateInput<T extends FieldValues = FieldValues>(
  ...
  return useInputHelpers(fieldName, {
    formatter: (value: unknown) => value instanceof Date ? Formatters.date(value, locale) : String(value),
...
export function usePhoneInput<T extends FieldValues = FieldValues>(
  ...
  return useInputHelpers(fieldName, {
    formatter: (value: unknown) => Formatters.phone(String(value || '')),
...
export function usePercentageInput<T extends FieldValues = FieldValues>(
  ...
  return useInputHelpers(fieldName, {
    formatter: (value: unknown) => Formatters.percentage(typeof value === 'number' ? value : 0, locale),
...
export function useSiretInput<T extends FieldValues = FieldValues>(
  ...
  return useInputHelpers(fieldName, {
    formatter: (value: unknown) => Formatters.siret(String(value || '')),
...
export function useFormPersistence<T extends FieldValues = FieldValues>(
  ...
  useEffect(() => {
    try {
      ...
      logger.warn('FormHelpers', 'Erreur lors de la restauration des données du formulaire:', error);
    }
    ...
    const subscription = form.watch((data) => {
      ...
      timeoutId = setTimeout(() => {
        try {
          ...
          logger.warn('FormHelpers', 'Erreur lors de la sauvegarde des données du formulaire:', error);
        }
      }, debounceMs);
    });
    return () => {
  ...
  const clearPersistedData = useCallback(() => {
    try {
      ...
      logger.warn('FormHelpers', 'Erreur lors de la suppression des données persistées:', error);
    }
  }, [storage, key]);
