...
export class ServerSitemapService {
  ...
  async updateWithDynamicRoutes(fetchRoutes: () => Promise<SitemapRoute[]>): Promise<void> {
    try {
      ...
      logger.error('SitemapGenerator', 'Erreur lors de la mise à jour du sitemap:', error instanceof Error ? error.message : String(error));
    }
  }
}
...
export const generateBuildSitemap = async () => {
  if (typeof window === 'undefined') {
    ...
    try {
      ...
      logger.warn('SitemapGenerator', '✅ Sitemap et robots.txt générés avec succès');
    } catch (error) {
      logger.error('SitemapGenerator', '❌ Erreur lors de la génération du sitemap:', error instanceof Error ? error.message : String(error));
    }
  }
};
