/**
 * Générateur Excel pour rapports comptables et fiscaux
 * Utilise ExcelJS pour créer des fichiers .xlsx professionnels
 */
...
export class ExcelGenerator {
  ...
  constructor(config: ExcelReportConfig) {
    ...
    this.workbook.creator = 'CassKai';
    this.workbook.created = new Date();
    this.workbook.modified = new Date();
    this.workbook.company = config.company.name;
  }
   ...
   */
  private applyTotalStyle(row: ExcelJS.Row): void {
    row.font = { bold: true };
    row.fill = {
      type: 'pattern',
      pattern: 'solid',
  ...
  /**
   * Ajouter l'en-tête d'entreprise
   */
  private addCompanyHeader(worksheet: ExcelJS.Worksheet): void {
    // Titre
    ...
    if (this.config.subtitle) {
      ...
      periodCell.value = this.config.subtitle;
  ...
  /**
   * Générer un Bilan (Balance Sheet) en Excel
   */
  public static async generateBalanceSheet(
    ...
    const workbook = generator.workbook;
    ...
    // ACTIF
    worksheet.addRow(['ACTIF']).font = { bold: true, size: 14 };
    worksheet.addRow([]);

    // Actif Immobilisé
    worksheet.addRow(['ACTIF IMMOBILISÉ']).font = { bold: true, size: 12 };
    const headerRow = worksheet.addRow(['Compte', 'Libellé', 'Montant Brut', 'Amortissements', 'Montant Net']);
    ...
    // Stocks
    worksheet.addRow([]);
    worksheet.addRow(['Stocks et en-cours']).font = { bold: true };
    if (data.assets.inventory) {
      data.assets.inventory.forEach(inv => {
        const row = worksheet.addRow({
    ...
    // Créances
    worksheet.addRow([]);
    worksheet.addRow(['Créances']).font = { bold: true };
    if (data.assets.receivables) {
      data.assets.receivables.forEach(rec => {
        const row = worksheet.addRow({
    ...
    // Disponibilités
    worksheet.addRow([]);
    worksheet.addRow(['Disponibilités']).font = { bold: true };
    if (data.assets.cash) {
      data.assets.cash.forEach(cash => {
        const row = worksheet.addRow({
    ...
    // Total Actif
    worksheet.addRow([]);
    const totalActifRow = worksheet.addRow(['', 'TOTAL ACTIF', '', '', data.totals.total_assets]);
    generator.applyTotalStyle(totalActifRow);
    totalActifRow.getCell(5).numFmt = '#,##0.00 €';

    // PASSIF (nouvelle feuille)
    const passifSheet = workbook.addWorksheet('Bilan Passif');
    generator.addCompanyHeader(passifSheet);

    passifSheet.addRow(['PASSIF']).font = { bold: true, size: 14 };
    passifSheet.addRow([]);

    // Capitaux propres
    passifSheet.addRow(['CAPITAUX PROPRES']).font = { bold: true, size: 12 };
    const passifHeader = passifSheet.addRow(['Compte', 'Libellé', 'Montant']);
    generator.applyHeaderStyle(passifHeader);
    ...
    if (data.equity.capital) {
      data.equity.capital.forEach(cap => {
        const row = passifSheet.addRow({
          account: cap.account_number,
          name: cap.account_name,
          amount: cap.balance
        });
        row.getCell(3).numFmt = '#,##0.00 €';
      });
    ...
    // Dettes
    passifSheet.addRow([]);
    passifSheet.addRow(['DETTES']).font = { bold: true };
    if (data.liabilities.payables) {
      data.liabilities.payables.forEach(pay => {
        const row = passifSheet.addRow({
    ...
    // Total Passif
    passifSheet.addRow([]);
    const totalPassifRow = passifSheet.addRow(['', 'TOTAL PASSIF', data.totals.total_liabilities + data.totals.total_equity]);
    generator.applyTotalStyle(totalPassifRow);
    totalPassifRow.getCell(3).numFmt = '#,##0.00 €';
  ...
  /**
   * Générer un Compte de Résultat en Excel
   */
  public static async generateIncomeStatement(
    ...
    const generator = new ExcelGenerator(config);
    const workbook = generator.workbook;
    const worksheet = workbook.addWorksheet('Compte de Résultat');
    ...
    // CHARGES
    worksheet.addRow(['CHARGES']).font = { bold: true, size: 14 };
    worksheet.addRow([]);

    const chargesHeader = worksheet.addRow(['Compte', 'Libellé', 'Montant']);
    generator.applyHeaderStyle(chargesHeader);

    // Charges d'exploitation
    if (data.expenses.purchases) {
      data.expenses.purchases.forEach(exp => {
        const row = worksheet.addRow({
          account: exp.account_number,
    ...
    if (data.expenses.external_charges) {
      data.expenses.external_charges.forEach(exp => {
        const row = worksheet.addRow({
          account: exp.account_number,
          name: exp.account_name,
          amount: exp.balance
    ...
    // Total Charges
    worksheet.addRow([]);
    const totalChargesRow = worksheet.addRow(['', 'TOTAL CHARGES', data.summary.total_expenses]);
    generator.applyTotalStyle(totalChargesRow);
    totalChargesRow.getCell(3).numFmt = '#,##0.00 €';

    // PRODUITS
    worksheet.addRow([]);
    worksheet.addRow([]);
    worksheet.addRow(['PRODUITS']).font = { bold: true, size: 14 };
    worksheet.addRow([]);

    const produitsHeader = worksheet.addRow(['Compte', 'Libellé', 'Montant']);
    generator.applyHeaderStyle(produitsHeader);

    // Produits d'exploitation
    if (data.revenue.sales) {
      data.revenue.sales.forEach(rev => {
        const row = worksheet.addRow({
          account: rev.account_number,
          name: rev.account_name,
          amount: rev.balance
        });
        row.getCell(3).numFmt = '#,##0.00 €';
      });
    ...
    // Total Produits
    worksheet.addRow([]);
    const totalProduitsRow = worksheet.addRow(['', 'TOTAL PRODUITS', data.summary.total_revenue]);
    generator.applyTotalStyle(totalProduitsRow);
    totalProduitsRow.getCell(3).numFmt = '#,##0.00 €';

    // RÉSULTAT NET
    worksheet.addRow([]);
    worksheet.addRow([]);
    const resultatRow = worksheet.addRow(['', 'RÉSULTAT NET', data.summary.net_income]);
    resultatRow.font = { bold: true, size: 12 };
    resultatRow.fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: data.summary.net_income >= 0 ? 'FF90EE90' : 'FFFFCCCB' }
    };
    resultatRow.getCell(3).numFmt = '#,##0.00 €';

    // Marge nette
    const margeRow = worksheet.addRow(['', `Marge nette: ${data.summary.margin_percentage.toFixed(2)}%`, '']);
    margeRow.font = { italic: true };
  ...
  /**
   * Générer une Balance Générale en Excel
   */
  public static async generateTrialBalance(
    ...
    // En-tête
    const headerRow = worksheet.addRow(['Compte', 'Libellé', 'Débit', 'Crédit', 'Solde']);
    generator.applyHeaderStyle(headerRow);
    ...
    data.accounts.forEach(account => {
      ...
      // Colorer le solde selon signe
      if (account.balance < 0) {
        row.getCell(5).font = { color: { argb: 'FFDC3545' } };
      }
    });
    ...
    worksheet.addRow([]);
    const verificationRow = worksheet.addRow([
      '',
      data.totals.balanced ? '✓ Balance équilibrée' : '✗ Balance déséquilibrée',
      '',
      '',
      ''
    ]);
    verificationRow.font = {
      bold: true,
      color: { argb: data.totals.balanced ? 'FF28A745' : 'FFDC3545' }
    };
  ...
  public static async generateGeneralLedger(
    ...
    // En-tête
    const headerRow = worksheet.addRow(['Date', 'Compte', 'Description', 'Débit', 'Crédit', 'Solde']);
    generator.applyHeaderStyle(headerRow);

    // Regrouper par compte
    const groupedEntries = data.entries.reduce((acc, entry) => {
      if (!acc[entry.account_number]) {
        acc[entry.account_number] = {
          name: entry.account_name,
          entries: []
        };
      }
      acc[entry.account_number].entries.push(entry);
      return acc;
    }, {} as Record<string, { name: string; entries: any[] }>);

    // Ajouter les entrées par compte
    Object.entries(groupedEntries).forEach(([accountNumber, accountData]) => {
      // Séparateur de compte
      ...
      accountRow.font = { bold: true, color: { argb: 'FF2C3E50' } };
      accountRow.fill = {
        type: 'pattern',
        pattern: 'solid',
      ...
      accountData.entries.forEach(entry => {
        const row = worksheet.addRow({
          date: generator.formatDate(entry.entry_date),
          account: '',
      ...
      worksheet.addRow([]); // Ligne vide entre comptes
    });
