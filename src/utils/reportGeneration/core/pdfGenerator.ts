...
export class PDFGenerator {
   ...
   * Ajouter un tableau
   */
  protected addTable(headers: string[], rows: (string | number)[][], options?: {
    startY?: number;
    highlightLastRow?: boolean;
    compact?: boolean;
  }): void;
  protected addTable(headers: string[], rows: (string | number)[][], startY?: number): void;
  protected addTable(headers: string[], rows: (string | number)[][], optionsOrStartY?: any): void {
    const options = typeof optionsOrStartY === 'number' ? { startY: optionsOrStartY } : optionsOrStartY || {};
    ...
    const tableOptions: any = {
      ...
      styles: {
        ...
        cellPadding: options.compact ? 2 : 3
      },
      headStyles: {
    ...
    // Mettre en évidence la dernière ligne si demandé
    if (options.highlightLastRow && rows.length > 0) {
      tableOptions.rowStyles = {};
      tableOptions.rowStyles[rows.length - 1] = {
  ...
  protected addCompanyInfo(): void {
    ...
    // Nom de l'entreprise
    this.doc.text(company.name, 15, this.currentY);

    // Adresse
    if (company.address) {
      this.currentY += 5;
      this.doc.text(company.address, 15, this.currentY);
    }
    ...
    if (company.siret) {
      ...
      this.doc.setTextColor(108, 117, 125);
      this.doc.text(`SIRET: ${company.siret}`, 15, this.currentY);
    }
   ...
   */
  protected addText(text: string, fontSize: number = 10, newLine: boolean = true): void {
    this.doc.setFontSize(fontSize);
    this.doc.setFont('helvetica', 'normal');
    this.doc.setTextColor(33, 37, 41);

    const lines = this.doc.splitTextToSize(text, 180);
    this.doc.text(lines, 15, this.currentY);

    if (newLine) {
      this.currentY += (lines.length * fontSize * 0.4) + 3;
    }
  }
  ...
  public static generateVATDeclaration(
    ...
    if (data.validation_message) {
      ...
      pdf.doc.setTextColor(data.is_valid ? 0 : 255, data.is_valid ? 128 : 0);
