...
export class MigrationChecker {
  ...
  async checkMigrationStatus(): Promise<MigrationStatus> {
    ...
    try {
      ...
      logger.error('MigrationChecker', '❌ Erreur lors de la vérification des migrations:', errorMessage);
  ...
  private async checkRequiredTables(): Promise<{ allPresent: boolean; missing: string[] }> {
    const checks = this.requiredTables.map(async (tableName) => {
      try {
        ...
        if (error) {
          logger.warn('MigrationChecker', `Avertissement pour la table ${tableName}: ${error.message}`);
        }
  ...
  logDetailedReport(status: MigrationStatus): void {
    ...
    if (overallStatus === 'error') {
      logger.error('MigrationChecker', 'Error occurred', report.join('\n'));
    } else {
      logger.warn('MigrationChecker', 'Warning', report.join('\n'));
    }
  }
  /**
