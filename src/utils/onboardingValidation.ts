...
class OnboardingValidator {
  ...
  async runAllTests(): Promise<{ summary: string; results: ValidationResult[] }> {
    logger.debug('OnboardingValidation', '🚀 Démarrage de la validation du flux onboarding...\n');
    // Exécuter les tests
    ...
    logger.debug('OnboardingValidation', `\n📊 Résumé des tests:\n${summary}`);
    if (failed === 0) {
      logger.debug('OnboardingValidation', '✅ Tous les tests sont passés ! Les améliorations semblent fonctionnelles.');
    } else {
      logger.error('OnboardingValidation', '❌ Certains tests ont échoué. Vérifiez les détails ci-dessus.');
    }
