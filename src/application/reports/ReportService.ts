...
export class ReportService {
  ...
  async generateReport(reportId: string, _parameters: ReportParameters): Promise<ReportExecution> {
    ...
    const validation = generator.validateParameters(report, _parameters);
    ...
    const cachedResult = await this.repository.getCachedReportResult(reportId, _parameters);

    if (cachedResult) {

      const execution = await this.repository.createExecution(reportId, _parameters);
    ...
    try {
      ...
      this.generateReportAsync(execution.id, report, generator, _parameters);
  ...
  private async generateReportAsync(
    ...
    try {
      ...
      const result = await generator.generateReport(report, _parameters);
      ...
      await this.repository.setCachedReportResult(report.metadata.id, _parameters, result, cacheTTL);
  ...
  async estimateReportGeneration(reportId: string, _parameters: ReportParameters): Promise<{
    ...
    const estimatedTime = generator.estimateGenerationTime(report, _parameters);
  ...
  async getReportInsights(reportId: string, _parameters: ReportParameters): Promise<{
