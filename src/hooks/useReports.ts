...
export function useReports(companyId: string): UseReportsReturn {
  ...
  const saveReport = useCallback(async (
    ...
    logger.warn('UseReports', 'saveReport is deprecated, use the new report execution system');
    return null;
  }, [user, companyId]);
  const deleteReport = useCallback(async (_reportId: string): Promise<void> => {
    if (!user || !companyId) return;
    // Legacy method - not implemented in new architecture
    logger.warn('UseReports', 'deleteReport is deprecated, use the new report execution system');
  }, [user, companyId]);
