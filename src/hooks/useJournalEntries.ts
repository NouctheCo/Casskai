...
export function useJournalEntries(companyId: string) {
  ...
  const createJournalEntry = useCallback(async (entryData: CreateJournalEntryData): Promise<JournalEntry | null> => {
    ...
    try {
      ...
      logger.warn('UseJournalEntries', '🔍 [useJournalEntries] createJournalEntry - journalId initial:', journalId);
      if (!journalId) {
        try {
          logger.warn('UseJournalEntries', '🔍 [useJournalEntries] createJournalEntry - Récupération journal par défaut pour company:', companyId);
          const { data: defaultJournal, error: journalError } = await supabase
            ...
            .limit(1)
            .single();
          logger.warn('UseJournalEntries', '🔍 [useJournalEntries] createJournalEntry - Résultat query journals:', { data: defaultJournal, error: journalError });
          if (journalError) {
            logger.error('UseJournalEntries', '❌ [useJournalEntries] createJournalEntry - Erreur récupération journal:', journalError);
            throw new Error(`Erreur récupération journal: ${journalError.message}`);
          }
          if (!defaultJournal) {
            logger.error('UseJournalEntries', '❌ [useJournalEntries] createJournalEntry - Aucun journal trouvé');
            throw new Error('Aucun journal actif trouvé pour cette entreprise');
          }
          journalId = defaultJournal.id;
          logger.warn('UseJournalEntries', '✅ [useJournalEntries] createJournalEntry - Journal par défaut trouvé:', defaultJournal);
        } catch (err) {
          logger.error('UseJournalEntries', '💥 [useJournalEntries] createJournalEntry - Exception récupération journal:', err);
          throw err;
        }
      }
      logger.warn('UseJournalEntries', '🔍 [useJournalEntries] createJournalEntry - journalId final:', journalId);
      ...
      logger.warn('UseJournalEntries', '🔍 [useJournalEntries] createJournalEntry - Payload final pour insertion:', entryInsertData);
