...
export function useHR(): UseHRReturn {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        logger.debug('useHR', '[useHR] Auth check:', {
          hasSession: !!session,
        ...
        logger.error('UseHR', '[useHR] Auth check failed:', err);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      logger.debug('UseHR', '[useHR] Auth state changed:', event, !!session);
  ...
  // Initial data load - only when auth is ready
  useEffect(() => {
    if (currentCompany?.id && isAuthReady) {
      logger.debug('UseHR', '[useHR] Loading initial data for company:', currentCompany.id);
      fetchMetrics();
      fetchEmployees();
    } else if (currentCompany?.id && !isAuthReady) {
      logger.warn('UseHR', '[useHR] Waiting for auth to be ready before loading data');
    }
  }, [currentCompany?.id, isAuthReady, fetchMetrics, fetchEmployees]);
  return {
    // Data
    employees,
    leaves,
