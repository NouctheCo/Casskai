...
export function useWidgetData(widgets: DashboardWidget[]) {
  ...
  const fetchWidgetData = async (widget: DashboardWidget) => {
    try {
      ...
      switch (widget.type) {
        case 'metric': {
          if (widget.title === 'Revenus du mois') {
            ...
            const totalRevenue = invoiceData?.reduce((acc, invoice) => acc + (parseFloat(invoice.total_incl_tax || invoice.total_amount) || 0), 0) || 0;
        ...
        case 'table': {
          const { data: invoiceData, error } = await supabase
            .from('invoices')
            .select('invoice_number, total_incl_tax, total_amount, status, created_at')
            .eq('status', 'En attente')
            .order('created_at', { ascending: false })
            .limit(5);
          if (error) throw error;
          // Map total_incl_tax to total_amount for display compatibility
          const mappedData = invoiceData?.map(inv => ({
            ...inv,
            total_amount: inv.total_incl_tax || inv.total_amount
          }));
          data = { invoices: mappedData };
          break;
        }
        case 'chart': {
          const { data: invoiceData, error } = await supabase
            .from('invoices')
            .select('created_at, total_incl_tax, total_amount')
      ...
      logger.error('UseWidgetData', `Error fetching data for widget ${widget.id}:`, error);
