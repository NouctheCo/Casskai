 ...
 */
import { useContext, useState, useEffect, ComponentType } from 'react';
import type { Module as _Module } from '@/types/modules.types';
  ...
  useEffect(() => {
    ...
    const loadComponents = async () => {
      ...
      try {
        ...
        if (module && module.getComponents) {
          ...
          const componentPromises = Object.entries(moduleComponents).map(async ([name, importFn]) => {
            try {
              ...
              logger.error('Modules.hooks', `[useModuleComponents] Erreur chargement composant ${name}:`, error);
              return null;
            }
          });
        ...
        logger.error('Modules.hooks', `[useModuleComponents] Erreur chargement composants ${moduleId}:`, error);
      } finally {
        setIsLoading(false);
      }
    };
