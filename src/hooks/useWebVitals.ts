...
export const useWebVitals = (options?: {
  ...
  const trackINP = useCallback((callback: (metric: WebVitalMetric) => void) => {
    ...
    try {
      ...
      logger.warn('UseWebVitals', '[WebVitals] INP tracking non supporté:', e);
    }
  }, []);
  // Callback pour traiter les métriques
  const handleMetric = useCallback((metric: WebVitalMetric) => {
    if (options?.debug) {
      logger.debug('useWebVitals', `[WebVitals] ${metric.name}:`, {
        value: metric.value,
        rating: metric.rating,
        delta: metric.delta,
      });
  ...
  setIsLoading(false);
        ...
        logger.warn('UseWebVitals', '[WebVitals] Erreur lors du chargement de web-vitals:', error);
        setIsLoading(false);
      }
    };
  void initWebVitals();
