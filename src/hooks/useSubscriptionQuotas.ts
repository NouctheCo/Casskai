...
export const useSubscriptionQuotas = () => {
  ...
  const loadUsageLimits = useCallback(async () => {
    ...
    try {
      ...
      logger.error('UseSubscriptionQuotas', 'Error loading usage limits:', error);
    } finally {
      setIsLoading(false);
    }
  }, [user?.id]);
  ...
  const canAccessFeature = useCallback(async (featureName: string): Promise<FeatureAccess> => {
    ...
    try {
      ...
      logger.error('UseSubscriptionQuotas', 'Error checking feature access:', error);
  ...
  const incrementUsageWithCheck = useCallback(async (
    ...
    try {
      ...
      logger.error('UseSubscriptionQuotas', 'Error incrementing usage:', error);
      return false;
    }
