  ...
  useEffect(() => {
    const loadCountries = async () => {
      try {
        ...
        logger.error('UseReferentials', 'Error occurred', error);
      } finally {
        setLoading(false);
      }
    };
...
export function useSectors() {
  ...
  const searchSectors = useCallback(async (searchTerm: string) => {
    try {
      ...
      logger.error('UseReferentials', '...', error);
      return sectors;
    }
  }, [sectors]);
...
export function useOnboardingReferentials() {
  ...
  const getCountryConfiguration = useCallback(async (countryCode: string) => {
    try {
      ...
      logger.error('UseReferentials', 'Erreur configuration pays:', error);
      return null;
    }
  }, []);
  return {
...
export function useCountryAutoConfig() {
  ...
  const loadCountryConfig = useCallback(async (countryCode: string) => {
    try {
      ...
      logger.error('UseReferentials', 'Erreur auto-config pays:', error);
      setConfig(null);
    } finally {
      setLoading(false);
    }
