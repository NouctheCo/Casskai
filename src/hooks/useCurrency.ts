...
export function useCurrency(defaultCurrency = 'XOF'): UseCurrencyReturn {
  ...
  const formatAmount = useCallback((
    ...
    try {
      ...
      logger.warn('UseCurrency', 'Erreur formatage montant:', errorMessage);
  ...
  useEffect(() => {
    const initializeRates = async () => {
      try {
        ...
        const errorMessage = error instanceof Error ? (error as Error).message : String(error);
        logger.warn('UseCurrency', 'Impossible de mettre à jour les taux:', errorMessage);
  ...
  const convertAmount = useCallback(async (
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? (error as Error).message : 'Erreur de conversion inconnue';
  ...
  const convertAmountSync = useCallback((
    ...
    try {
      ...
      logger.warn('UseCurrency', 'Erreur conversion synchrone:', error);
      return amount;
    }
  }, [currencyService, baseCurrency]);
  // Conversion en lot
  const convertBatch = useCallback(async (
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? (error as Error).message : 'Erreur de conversion batch';
      setError(errorMessage);
      throw error;
    } finally {
      setIsLoading(false);
  ...
  const getExchangeRate = useCallback(async (from: string, to: string): Promise<number> => {
    try {
      ...
      const errorMessage = error instanceof Error ? (error as Error).message : 'Erreur récupération taux';
      setError(errorMessage);
      throw error;
    } finally {
      setIsLoading(false);
  ...
  const refreshRates = useCallback(async (): Promise<void> => {
    try {
      ...
      const errorMessage = error instanceof Error ? (error as Error).message : 'Erreur de rafraîchissement des taux';
      setError(errorMessage);
...
export const useCurrencyStats = () => {
  ...
  const getPopularRates = useCallback(async (forceRefresh = false) => {
    try {
      ...
      logger.warn('UseCurrency', 'Impossible de récupérer les taux populaires:', error);
      return stats.popularRates;
    }
