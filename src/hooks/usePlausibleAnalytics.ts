...
class PlausibleService {
  ...
  private async initializePlausible(): Promise<void> {
    ...
    if (
      ...
      logger.debug('UsePlausibleAnalytics', '[Plausible] Tracking désactivé en développement local');
      return;
    }
    try {
      ...
      script.onload = () => {
        this.isLoaded = true;
        this.processQueue();
        logger.debug('UsePlausibleAnalytics', '[Plausible] Analytics chargé avec succès');
      };
      script.onerror = () => {
        logger.error('UsePlausibleAnalytics', '[Plausible] Échec du chargement du script analytics');
      };
      ...
      logger.error('UsePlausibleAnalytics', '[Plausible] Erreur lors de l\'initialisation:', error);
    }
  }
      ...
      try {
        if (this.isLoaded) {
          ...
          if (window.plausible && typeof window.plausible === 'function') {
            return (window.plausible as any).apply(null, args);
          }
        } else {
          ...
          this.queue.push(() => {
            try {
              if (window.plausible && typeof window.plausible === 'function') {
                (window.plausible as any).apply(null, args);
              }
            } catch (error) {
              logger.warn('UsePlausibleAnalytics', '[Plausible] Erreur lors de l\'envoi des analytics:', error);
            }
          });
        }
      } catch (error) {
  ...
  trackPageview(url?: string): void {
    ...
    try {
      ...
      if (window.plausible) {
        window.plausible('pageview', { u: pageUrl } as any);
      }
    } catch (error) {
      logger.warn('UsePlausibleAnalytics', '[Plausible] Erreur lors du tracking de page vue:', error);
    }
  }
  ...
  trackEvent(eventName: string, props?: PlausibleEventProps): void {
    ...
    try {
      ...
      logger.warn('UsePlausibleAnalytics', '[Plausible] Erreur lors du tracking d\'événement:', error);
    }
  }
  ...
  trackGoal(goalName: string, props?: PlausibleGoalProps): void {
    ...
    try {
      ...
      logger.warn('UsePlausibleAnalytics', '[Plausible] Erreur lors du tracking de goal:', error);
    }
  }
