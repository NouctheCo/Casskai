...
import { MessageCircle, Zap, Clock, CheckCircle, Phone } from 'lucide-react';
...
export function WhatsAppChat({
  ...
  const openWhatsApp = async () => {
    ...
    sendToN8nWebhook(webhookData).catch(error => {
      logger.warn('WhatsAppChat', 'N8n webhook failed:', error);
    });
    // Analytics/tracking
...
export function WhatsAppFloatingButton({
  ...
  const openWhatsApp = async () => {
    ...
    try {
      ...
      logger.error('WhatsAppChat', 'WhatsApp error:', error instanceof Error ? error.message : String(error));
      toast({
        title: "Erreur",
