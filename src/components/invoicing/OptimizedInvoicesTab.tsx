...
const OptimizedInvoicesTab: React.FC<OptimizedInvoicesTabProps> = ({ shouldCreateNew, onCreateNewCompleted }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.warn('No data loaded (this is normal for empty database):', error instanceof Error ? error.message : String(error));
  ...
  const filteredInvoices = invoices.filter(invoice => {
    ...
    const matchesSearch = !searchTerm || 
      ...
      ((invoice.third_party || invoice.client) as { name?: string })?.name?.toLowerCase().includes(searchTerm.toLowerCase());
                    ...
                    <TableRow key={invoice.id as string} className="hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900/30">
                      ...
                      <TableCell>
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="font-medium">{((invoice.third_party || invoice.client) as { name?: string })?.name || 'Client supprimé'}</p>
                      ...
                      <TableCell>
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="font-medium">{formatCurrency(invoice.total_amount as number)}</p>
  ...
  useEffect(() => {
    ...
    if (open && !invoice) {
      ...
      setFormData({
        ...
        clientId: (invoice.third_party_id as string) || '',
