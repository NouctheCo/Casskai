      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <div>
              <Label className="text-sm font-medium text-gray-600 dark:text-gray-300">Nom complet</Label>
              <p className="text-lg font-semibold">{client.name}</p>
            </div>
            <div>
              <Label className="text-sm font-medium text-gray-600 dark:text-gray-300">Entreprise</Label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {client.phone && (
                <div className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
                  ...
                  <div>
                    <Label className="text-sm font-medium text-gray-600 dark:text-gray-300">Téléphone</Label>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-md text-center">
                ...
                <Label className="text-sm font-medium text-gray-600 dark:text-gray-300">Factures</Label>
          ...
          <div className="border-t pt-4">
            <Label className="text-sm font-medium text-gray-600 dark:text-gray-300">Historique</Label>
            <div className="mt-2 space-y-2">
              <div className="flex items-center space-x-3 text-sm">
                <div className="w-2 h-2 bg-blue-500 rounded-full dark:bg-blue-900/20"></div>
                <span>Client créé le {new Date(client.createdAt || new Date().toISOString()).toLocaleDateString('fr-FR')}</span>
              </div>
    ...
    <TableRow className="hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:bg-gray-900/30">
      <TableCell>
        <div>
          ...
          {client.company && (
            <div className="text-sm text-gray-500 dark:text-gray-300">{client.company}</div>
          )}
        </div>
      </TableCell>
      <TableCell>
...
export default function OptimizedClientsTab() {
  ...
  const loadClients = async () => {
    try {
      ...
      const transformed = customers
        .filter(customer => customer.id) // Filtrer les clients sans ID
        .map(customer => ({
        id: customer.id!,
        name: customer.name,
        ...
        createdAt: ((customer as any).created_at && typeof (customer as any).created_at === 'string')
          ? (customer as any).created_at
          : new Date().toISOString()
      }));
      setClients(transformed);
    } catch (error) {
      logger.error('OptimizedClientsTab', 'Error loading clients:', error instanceof Error ? error.message : String(error));
  ...
  const handleSaveClient = async (clientData: Client) => {
    try {
      ...
      logger.error('OptimizedClientsTab', 'Error saving client:', error instanceof Error ? error.message : String(error));
      toast({
        title: 'Erreur',
  ...
  const handleDeleteClient = async (client: Client) => {
    ...
    try {
      ...
      logger.error('OptimizedClientsTab', 'Error deleting client:', error instanceof Error ? error.message : String(error));
      toast({
        title: 'Erreur',
