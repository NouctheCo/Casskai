...
export const HomePage: React.FC = () => {
  ...
  if (import.meta.env.DEV) {
    logger.debug('HomePage - Onboarding flow debug', {
      userId: user?.id,
      onboardingCompleted,
  ...
  if (user && !onboardingCompleted && !currentCompany && !hasLocalCompany) {
    if (import.meta.env.DEV) {
      logger.warn('Home', '🎯 HomePage: Redirecting to onboarding');
    }
  ...
  if (user && onboardingCompleted && !currentCompany && !hasLocalCompany) {
    if (import.meta.env.DEV) {
      logger.warn('Home', '⏳ HomePage: Onboarding completed but waiting for company data');
    }
