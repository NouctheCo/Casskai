...
import {
  ...
  CheckCircle, ArrowRight, ArrowLeft, Heart, Star,
...
export const AccountDeletionWizard: React.FC<DeletionWizardProps> = ({
  ...
  // ✅ Nouveaux states pour charger les vrais utilisateurs et permettre le skip transfer
  const [eligibleUsers, setEligibleUsers] = useState<Record<string, any[]>>({});
  ...
  const performAnalysis = async () => {
    try {
      ...
      setAnalysis(result);
      // ✅ Charger les utilisateurs éligibles pour chaque entreprise nécessitant un transfert
      if (result.requiresOwnershipTransfer && result.companiesAsSoleOwner?.length > 0) {
        setLoadingEligibleUsers(true);
        const usersMap: Record<string, any[]> = {};
        for (const company of result.companiesAsSoleOwner) {
          try {
            const users = await accountDeletionService.getEligibleTransferUsers(company.company_id);
            usersMap[company.company_id] = users;
          } catch (error) {
            logger.error('AccountDeletionWizard', `Erreur chargement utilisateurs pour ${company.company_name}:`, error);
            usersMap[company.company_id] = [];
          }
        }
        setEligibleUsers(usersMap);
      ...
      logger.error('AccountDeletionWizard', 'Erreur analyse:', error instanceof Error ? error.message : String(error));
    }
  };
  // ✅ Navigation conditionnelle avec logique de flux
  const getNextStep = (current: WizardStep): WizardStep | null => {
    switch (current) {
      case 'welcome': return 'analysis';
      case 'analysis': return null; // performAnalysis gère la transition
      case 'impact': return 'transfer';
      case 'transfer': return 'export';
      case 'export': return 'confirmation';
      case 'confirmation': return 'processing';
      case 'processing': return 'farewell';
      case 'farewell': return null;
      default: return null;
    }
  };
  const getPreviousStep = (current: WizardStep): WizardStep | null => {
    switch (current) {
      ...
      case 'analysis': return 'welcome';
      case 'impact': return null; // Pas de retour depuis l'analyse
      case 'transfer': return 'impact';
      case 'export':
        // Si on venait du transfer, on y retourne. Sinon on retourne à impact
        return analysis?.requiresOwnershipTransfer ? 'transfer' : 'impact';
      case 'confirmation': return 'export';
      case 'processing': return null; // Pas de retour pendant le processing
      case 'farewell': return null;
      default: return null;
    }
  };
  ...
  const processDeletion = async () => {
    ...
    try {
      ...
      logger.error('AccountDeletionWizard', 'Erreur suppression:', error instanceof Error ? error.message : String(error));
    } finally {
      setProcessing(false);
    }
  };
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 text-sm">
        <div className="flex flex-col items-center">
          <Building className="h-8 w-8 text-gray-400 dark:text-gray-500 mb-2" />
  ...
  const renderTransferStep = () => (
    ...
    >
              ...
              <div className="space-y-3">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    // ✅ Trouver l'email du user sélectionné
                    const selectedUser = (eligibleUsers[company.company_id] || []).find(u => u.user_id === value);
                    if (existingIndex >= 0) {
                      updatedPlans[existingIndex].new_owner_id = value;
                      updatedPlans[existingIndex].new_owner_email = selectedUser?.email || '';
                    } else {
                      updatedPlans.push({
                ...
                >
                  ...
                  <SelectContent>
                    {/* ✅ Affichage des vrais utilisateurs chargés */}
                    {loadingEligibleUsers ? (
                      <SelectItem value="" disabled>Chargement...</SelectItem>
                    ) : (eligibleUsers[company.company_id] || []).length > 0 ? (
                      (eligibleUsers[company.company_id] || []).map((user) => (
                        <SelectItem key={user.user_id} value={user.user_id}>
                          {user.email} - {user.role}
                        </SelectItem>
                      ))
                    ) : (
                      <SelectItem value="" disabled>Aucun utilisateur éligible</SelectItem>
                    )}
                  </SelectContent>
                </Select>
      ...
      {/* ✅ Option pour supprimer définitivement sans transfert */}
        ...
        <CardContent className="p-4">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <label
                htmlFor="skip-transfer"
                className="text-sm font-medium text-red-900 dark:text-red-100 cursor-pointer"
              >
                Supprimer définitivement sans transférer la propriété
              </label>
              <p className="text-xs text-red-700 dark:text-red-300 mt-1">
                ⚠️ En cochant cette case, toutes les données de l'entreprise seront définitivement supprimées
                après la période de grâce de 30 jours. Cette action est irréversible.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <div className="flex space-x-3">
        ...
        <Button
          ...
          className="flex-1"
          disabled={
            analysis?.companiesAsSoleOwner?.length > 0 &&
