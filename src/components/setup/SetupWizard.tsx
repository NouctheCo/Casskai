...
const LanguageSwitcher = ({ className = "" }) => {
  ...
  const handleLanguageChange = async (langCode: string) => {
    try {
      ...
      logger.error('SetupWizard', 'Erreur changement de langue:', error instanceof Error ? error.message : String(error));
    }
  };
  return (
...
const UniversalSetupWizard = () => {
  ...
  const detectUserMarket = async () => {
    try {
      ...
      const detected = marketMap[data.country_code as string] || 'france';
  ...
  const handleFinish = async () => {
    try {
      logger.debug('SetupWizard', 'Configuration finale:', config);
      // eslint-disable-next-line no-alert
      alert(t('setup.installation.completed'));
    } catch (error) {
      logger.error('SetupWizard', 'Erreur lors de l\'installation:', error instanceof Error ? error.message : String(error));
      // eslint-disable-next-line no-alert
