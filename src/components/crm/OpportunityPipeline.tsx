...
import type { Opportunity, OpportunityStageType } from '@/types/crm.types';
...
export const OpportunityPipeline: React.FC<OpportunityPipelineProps> = ({
  ...
  const opportunitiesByStage = PIPELINE_STAGES.reduce((acc, stage) => {
    ...
    return acc;
  }, {} as Record<OpportunityStageType, Opportunity[]>);
  // Calculer les statistiques pour chaque étape
  const getStageStats = (stageId: OpportunityStageType) => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const newStage = over.id as OpportunityStageType;
    ...
    try {
      ...
      logger.error('OpportunityPipeline', 'Error updating opportunity:', error instanceof Error ? error.message : String(error));
