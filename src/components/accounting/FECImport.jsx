...
import { useFECParser } from '@/lib/fecUtils';
...
const FECImport = ({ currentEnterpriseId }) => {
  ...
  const { parseFECFileContent } = useFECParser();
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const data = await parseFECFileContent(content);
          setParsedData(data);
        } catch (error) {
      ...
      reader.onerror = () => {
        toast({
          variant: 'destructive',
          title: t('error'),
          description: t('fecImport.error.readingFile', { defaultValue: 'Error reading file' })
        });
        setParsing(false);
      };
      ...
      toast({
        variant: 'destructive',
        title: t('error'),
        description: error.message || t('fecImport.error.handlingFile', { defaultValue: 'Error handling file' })
      });
      setParsing(false);
    }
  }, [parseFECFileContent, t, toast]);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const result = await fecImportService.importFECData(parsedData, currentEnterpriseId);
