...
const ForecastChartView2: React.FC<ForecastChartViewProps> = ({
  forecast,
  ...
  const prepareRechartsData = (): ChartDataPoint[] => {
    return months.map((month, index) => ({
      period: month,
      value: Math.round((forecast.total_revenue / 12) * (1 + Math.sin(index / 2) * 0.1)),
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 ${className}`}>
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Évolution Financière - {forecast.name}</h3>
        <p className="text-sm text-gray-600 dark:text-gray-400">Projection mensuelle des revenus et dépenses pour l'année en cours</p>
      </div>
      ...
      <div style={{ width: '100%', height: 320 }}>
        <ResponsiveContainer>
          <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
            ...
            <XAxis dataKey="period" stroke="#6b7280" tick={{ fontSize: 12 }} axisLine={{ stroke: '#e5e7eb' }} />
            <YAxis stroke="#6b7280" tick={{ fontSize: 12 }} axisLine={{ stroke: '#e5e7eb' }} tickFormatter={(value) => formatCurrency(value)} />
            <Tooltip content={<CustomTooltip />} />
            <Legend wrapperStyle={{ paddingTop: '20px' }} iconType="line" />
            <Line type="monotone" dataKey="revenus" stroke="#10b981" strokeWidth={3} name="Revenus" dot={{ fill: '#10b981', strokeWidth: 2, r: 4 }} activeDot={{ r: 6, stroke: '#10b981', strokeWidth: 2 }} />
            <Line type="monotone" dataKey="depenses" stroke="#ef4444" strokeWidth={3} name="Dépenses" dot={{ fill: '#ef4444', strokeWidth: 2, r: 4 }} activeDot={{ r: 6, stroke: '#ef4444', strokeWidth: 2 }} />
            <Line type="monotone" dataKey="benefices" stroke="#3b82f6" strokeWidth={2} strokeDasharray="5 5" name="Bénéfices" dot={{ fill: '#3b82f6', strokeWidth: 2, r: 3 }} activeDot={{ r: 5, stroke: '#3b82f6', strokeWidth: 2 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-4">
        <div className="text-center p-3 bg-green-50 rounded-lg dark:bg-green-900/20">
          ...
          <p className="text-lg font-bold text-green-600">{formatCurrency(forecast.total_revenue)}</p>
        </div>
        ...
        <div className="text-center p-3 bg-red-50 rounded-lg dark:bg-red-900/20">
          ...
          <p className="text-lg font-bold text-red-600 dark:text-red-400">{formatCurrency(forecast.total_expenses)}</p>
        </div>
        ...
        <div className="text-center p-3 bg-blue-50 rounded-lg dark:bg-blue-900/20">
          ...
          <p className="text-lg font-bold text-blue-600">{formatCurrency(forecast.total_revenue - forecast.total_expenses)}</p>
        </div>
      </div>
    </div>
  );
...
export default ForecastChartView2;
