...
export const ExchangeRateWidget = ({ 
  ...
  const { getExchangeRate, formatAmount: _formatAmount } = useCurrency();
    ...
    const loadRates = async () => {
      ...
      for (const pair of pairs) {
        try {
          ...
          logger.error('CurrencyComponents', `Erreur taux ${pair.from}/${pair.to}:`, error instanceof Error ? error.message : String(error));
        }
      }
      setRates(newRates);
          ...
          return (
            <div key={key} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
              <div className="flex items-center space-x-2">
                ...
                <ArrowRightLeft className="h-3 w-3 text-gray-400 dark:text-gray-500" />
