...
export class OnboardingStorageService {
  ...
  private readSessionToken(userId: string): string | null {
    ...
    try {
      ...
      logger.warn('OnboardingStorage', 'Unable to read onboarding session token from localStorage:', error);
      return null;
    }
  }
  private persistSessionToken(userId: string, token: string): void {
    ...
    try {
      ...
      logger.warn('OnboardingStorage', 'Unable to persist onboarding session token:', error);
    }
  }
  private clearSessionToken(userId: string): void {
    ...
    try {
      ...
      logger.warn('OnboardingStorage', 'Unable to clear onboarding session token:', error);
    }
  }
  ...
  saveToLocalStorage(userId: string, data: OnboardingData): void {
    ...
    try {
      ...
      logger.warn('OnboardingStorage', 'Unable to save onboarding draft to localStorage:', error);
    }
  }
  /**
  ...
  getLocalStorageData(userId: string): OnboardingData | null {
    ...
    try {
      ...
      logger.warn('OnboardingStorage', 'Unable to read onboarding draft from localStorage:', error);
      return null;
    }
  }
  /**
  ...
  clearLocalStorageData(userId: string): void {
    ...
    try {
      ...
      logger.warn('OnboardingStorage', 'Unable to clear onboarding draft from localStorage:', error);
    }
  }
  /**
  ...
  private async fetchSupabaseSession(userId: string, forceRefresh = false): Promise<SupabaseOnboardingSession | null> {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) {
      logger.error('OnboardingStorage', 'Unable to fetch onboarding session from Supabase:', error);
  ...
  private async ensureSupabaseSession(userId: string, data: OnboardingData): Promise<SupabaseOnboardingSession | null> {
    ...
    if (existing) {
      const { data: updated, error } = await supabase
        ...
        .single();
      if (error) {
        logger.error('OnboardingStorage', 'Unable to update onboarding session in Supabase:', error);
        return null;
      }
    ...
    const { data: inserted, error } = await supabase
      ...
      .single();
    if (error) {
      logger.error('OnboardingStorage', 'Unable to create onboarding session in Supabase:', error);
      return null;
    }
  ...
  async getActiveSession(userId: string): Promise<OnboardingResponse<OnboardingSession | null>> {
    try {
      ...
      logger.error('OnboardingStorage', 'Error while retrieving onboarding session:', error);
      return {
        success: false,
  ...
  async saveOnboardingData(userId: string, data: OnboardingData): Promise<OnboardingResponse<OnboardingData>> {
    try {
      ...
      logger.error('OnboardingStorage', 'Error while saving onboarding data:', error);
      return {
        success: false,
    ...
    try {
      ...
      if (session) {
        ...
        const { error } = await supabase
          ...
          .eq('id', session.id);
        if (error) {
          logger.error('OnboardingStorage', 'Unable to deactivate onboarding session in Supabase:', error);
        }
      }
      ...
      logger.error('OnboardingStorage', 'Error while clearing onboarding data:', error);
      return {
        success: false,
  ...
  async getOnboardingData(userId: string): Promise<OnboardingResponse<OnboardingData | null>> {
    try {
      ...
      logger.error('OnboardingStorage', 'Error while fetching onboarding data:', error);
      return {
        success: false,
