...
class FinancialReportsService {
  ...
  private async calculateAccountBalances(filters: ReportFilters): Promise<AccountBalance[]> {
    ...
    if (accountsError) {
      logger.error('FinancialReports', 'Error fetching accounts:', accountsError);
      return [];
    }
    ...
    if (entriesError) {
      logger.error('FinancialReports', 'Error fetching journal entries:', entriesError);
      return [];
    }
    ...
    return (accounts || []).map(account => {
      ...
      const periodDebit = balance.debit;  // Maintenant ce sont les vrais mouvements de période
      const periodCredit = balance.credit;  // Maintenant ce sont les vrais mouvements de période
  ...
  async generateAndSaveReport(
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .single();
      if (error) {
        logger.error('FinancialReports', 'Error saving report:', error);
        return { success: false, error: error.message };
      }
      return { success: true, reportId: data.id };
    } catch (error) {
      logger.error('FinancialReports', 'Error generating report:', error);
      return {
        success: false,
