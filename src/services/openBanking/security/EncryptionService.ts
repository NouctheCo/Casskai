...
export class EncryptionService {
  ...
  async initialize(masterKeyMaterial: string | ArrayBuffer): Promise<void> {
    try {
      ...
      logger.warn('Encryption', 'Encryption service initialized successfully');
    } catch (error: unknown) {
  ...
  async verifyHMACSignature(
    ...
    try {
      ...
      logger.error('Encryption', 'HMAC verification failed:', error);
      return false;
    }
  }
  ...
  private async auditLog(action: string, userId: string, metadata: any): Promise<void> {
    try {
      ...
      logger.warn('Encryption', 'Audit log:', auditEntry);
    } catch (error: unknown) {
      logger.error('Encryption', 'Failed to create audit log:', error);
    }
  }
  // Getters
...
export class TokenRotationService {
  ...
  startAutoRotation(callback: () => Promise<void>): void {
    ...
    this.rotationTimer = setInterval(async () => {
      try {
        await callback();
        logger.warn('Encryption', 'Token rotation completed successfully');
      } catch (error: unknown) {
        logger.error('Encryption', 'Token rotation failed:', error);
      }
  ...
  async forceRotation(callback: () => Promise<void>): Promise<void> {
    try {
      await callback();
      logger.warn('Encryption', 'Forced token rotation completed successfully');
    } catch (error: unknown) {
      logger.error('Encryption', 'Forced token rotation failed:', error);
      throw error;
    }
  }
  dispose(): void {
