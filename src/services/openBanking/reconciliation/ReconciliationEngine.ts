...
export class ReconciliationEngine {
  ...
  async initialize(rules: ReconciliationRule[]): Promise<void> {
    try {
      ...
      logger.warn('ReconciliationEngine', `Reconciliation engine initialized with ${rules.length} rules`);
    } catch (error: unknown) {
  ...
  private async applyMatches(matches: ReconciliationMatch[]): Promise<void> {
    for (const match of matches) {
      if (match.status === 'matched' && match.confidence > 0.8) {
        ...
        logger.warn('ReconciliationEngine', `Auto-applied match: ${match.id}`);
      }
    }
  }
