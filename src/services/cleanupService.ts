...
export class CleanupService {
  ...
  static async cleanupDemoData(companyId: string): Promise<{ success: boolean; message: string; deletedCount?: number }> {
    try {
      ...
      if (selectError) {
        logger.error('Cleanup', 'Erreur lors de la récupération des écritures à supprimer:', selectError);
        throw selectError;
      }
      ...
      if (itemsError) {
        logger.error('Cleanup', 'Erreur lors de la suppression des lignes d\'écritures:', itemsError);
        throw itemsError;
      }
      ...
      const { error: entriesError } = await supabase
        ...
        .in('id', entryIds);
      if (entriesError) {
        logger.error('Cleanup', 'Erreur lors de la suppression des écritures:', entriesError);
        throw entriesError;
      }
      return {
        success: true,
      ...
      logger.error('Cleanup', 'Erreur lors du nettoyage des données d\'exemple:', error instanceof Error ? error.message : String(error));
      return {
        success: false,
  ...
  static async hasDemoData(companyId: string): Promise<boolean> {
    try {
      ...
      logger.error('Cleanup', 'Erreur lors de la vérification des données d\'exemple:', error instanceof Error ? error.message : String(error));
      return false;
    }
  }
}