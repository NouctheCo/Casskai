...
class AIAnalyticsService {
  ...
  async initialize(): Promise<void> {
    try {
      logger.warn('AiAnalytics', 'Initializing AI Analytics Service...');
      ...
      logger.warn('AiAnalytics', 'AI Analytics Service initialized successfully');
    } catch (error: unknown) {
      logger.error('AiAnalytics', 'Failed to initialize AI Analytics Service:', error);
      throw error;
    }
  }
  ...
  private async initializeModels(): Promise<void> {
    try {
      ...
      logger.error('AiAnalytics', 'Error initializing models:', error);
  ...
  async detectAnomalies(transactions: Transaction[]): Promise<AIServiceResponse<AnomalyDetection[]>> {
    try {
      ...
      logger.error('AiAnalytics', 'Error detecting anomalies:', error);
      return {
        success: false,
  ...
  async categorizeExpense(transaction: Transaction, categories: ExpenseCategory[]): Promise<AIServiceResponse<CategoryPrediction>> {
    try {
      ...
      logger.error('AiAnalytics', 'Error categorizing expense:', error);
      return this.ruleBasedCategorization(transaction, categories);
    }
  }
  ...
  async predictCashFlow(historicalData: FinancialTimeSeriesData[], daysAhead: number = 30): Promise<AIServiceResponse<CashFlowPrediction[]>> {
    try {
      ...
      logger.error('AiAnalytics', 'Error predicting cash flow:', error);
      return this.statisticalCashFlowPrediction(historicalData, daysAhead);
    }
  }
  ...
  async calculateHealthScore(transactions: Transaction[], balances: number[]): Promise<AIServiceResponse<FinancialHealthScore>> {
    try {
      ...
      logger.error('AiAnalytics', 'Error calculating health score:', error);
      return {
        success: false,
        error: (error instanceof Error ? error.message : 'Une erreur est survenue')
      };
  ...
  async saveModels(): Promise<void> {
    try {
      ...
      logger.warn('AiAnalytics', 'Models metadata saved successfully');
    } catch (error: unknown) {
      logger.error('AiAnalytics', 'Error saving models:', error);
    }
  }
