...
export class ModuleManager {
  ...
  async initialize(context: Partial<ModuleContext>): Promise<void> {
    try {
      ...
      logger.warn('ModuleManager', '[ModuleManager] Initialisé avec succès');
    } catch (error: unknown) {
      logger.error('ModuleManager', '[ModuleManager] Erreur d\'initialisation:', error);
      throw error;
    }
  }
  ...
  registerModule(module: Module): void {
    ...
    if (this.modules.has(id)) {
      ...
      logger.warn('ModuleManager', `[ModuleManager] Module ${id} already registered - skipping duplicate registration`);
      return;
    }
  ...
  logger.warn('ModuleManager', `[ModuleManager] Module ${id} enregistré`);
  }
  // Activer un module
  ...
  logger.warn('ModuleManager', `[ModuleManager] Module ${moduleId} activé avec succès`);
    } catch (error: unknown) {
      logger.error('ModuleManager', `[ModuleManager] Erreur lors de l'activation de ${moduleId}:`, error);
  ...
  async deactivateModule(moduleId: string, userId: string): Promise<void> {
    ...
    try {
      ...
      this.activations.set(moduleId, activation);
      await this.saveActivation(activation);
  logger.warn('ModuleManager', `[ModuleManager] Module ${moduleId} désactivé avec succès`);
    } catch (error: unknown) {
      logger.error('ModuleManager', `[ModuleManager] Erreur lors de la désactivation de ${moduleId}:`, error);
  ...
  async updateModuleConfig(moduleId: string, config: Record<string, unknown>, _userId: string): Promise<void> {
    ...
    this.activations.set(moduleId, activation);
    await this.saveActivation(activation);
  logger.warn('ModuleManager', `[ModuleManager] Configuration du module ${moduleId} mise à jour`);
  }
  ...
  private async initializeCoreModules(_context: Partial<ModuleContext>): Promise<void> {
    ...
    results.forEach((res, idx) => {
      if (res.status === 'rejected') {
        ...
        logger.error('ModuleManager', `[ModuleManager] Erreur activation module core ${failed.definition.id}:`, res.reason);
      }
    });
  }
  ...
  private async loadActivations(tenantId: string): Promise<void> {
    try {
      ...
      if (supabase) {
        try {
          logger.warn('ModuleManager', '[ModuleManager] Loading modules from Supabase for tenant:', tenantId);
          const { data: modules, error } = await supabase
            .from('company_modules')
            .select('module_key, is_enabled')
            .eq('company_id', tenantId);
          if (!error && modules && modules.length > 0) {
            logger.warn('ModuleManager', '[ModuleManager] Found modules in Supabase:', modules.length);
            ...
            logger.warn('ModuleManager', '[ModuleManager] Modules synchronized from Supabase to localStorage');
            return;
          } else if (error) {
            logger.warn('ModuleManager', '[ModuleManager] Error loading from Supabase, falling back to localStorage:', (error instanceof Error ? error.message : 'Une erreur est survenue'));
          }
        } catch (supabaseError) {
          logger.warn('ModuleManager', '[ModuleManager] Supabase not available, using localStorage fallback:', supabaseError);
        }
      }
      // Fallback vers localStorage
      const simpleModules = localStorage.getItem('casskai_modules');
      if (simpleModules) {
        logger.warn('ModuleManager', '[ModuleManager] Loading modules from localStorage (casskai_modules)');
      ...
      if (stored) {
  logger.warn('ModuleManager', `[ModuleManager] Loading modules from complex format (casskai-modules-${tenantId})`);
        const activations = JSON.parse(stored) as ModuleActivation[];
        activations.forEach(activation => {
          this.activations.set(activation.moduleId, activation);
        });
      } else {
        logger.warn('ModuleManager', '[ModuleManager] No modules found, using default modules');
      ...
      logger.error('ModuleManager', '[ModuleManager] Error loading activations:', error);
  ...
  private async saveActivation(_activation: ModuleActivation): Promise<void> {
    try {
      ...
      logger.warn('ModuleManager', '[ModuleManager] Modules saved to localStorage in simple format');
      // Sauvegarder aussi dans Supabase si tenantId est disponible
      if (this.tenantId && supabase) {
        try {
          logger.warn('ModuleManager', '[ModuleManager] Saving modules to Supabase for tenant:', this.tenantId);
          ...
          if (modulesToUpsert.length > 0) {
            ...
            if (error) {
              logger.error('ModuleManager', '[ModuleManager] Error upserting to Supabase:', error);
            } else {
              logger.warn('ModuleManager', '[ModuleManager] Modules upserted to Supabase successfully');
            }
          }
        } catch (supabaseError) {
          logger.error('ModuleManager', '[ModuleManager] Supabase save error:', supabaseError);
        }
      }
    } catch (error: unknown) {
      logger.error('ModuleManager', '[ModuleManager] Erreur sauvegarde activation:', error);
    }
  }
