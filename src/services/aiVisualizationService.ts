...
class AIVisualizationService {
  ...
  initialize(): void {
    try {
      logger.warn('AiVisualization', 'Initializing AI Visualization Service...');
      // Configuration des échelles de couleurs
      this.setupColorSchemes();
      this.isInitialized = true;
      logger.warn('AiVisualization', 'AI Visualization Service initialized successfully');
    } catch (error: unknown) {
      logger.error('AiVisualization', 'Failed to initialize AI Visualization Service:', error);
      throw error;
    }
  }
  ...
  async generateHeatmapData(
    ...
    try {
      ...
      logger.error('AiVisualization', 'Error generating heatmap data:', error);
      return {
        success: false,
  ...
  async generateSankeyData(
    ...
    try {
      ...
      logger.error('AiVisualization', 'Error generating Sankey data:', error);
      return {
        success: false,
        error: (error instanceof Error ? error.message : 'Une erreur est survenue')
      };
  ...
  async generateTreemapData(
    ...
    try {
      ...
      logger.error('AiVisualization', 'Error generating treemap data:', error);
      return {
        success: false,
        error: (error instanceof Error ? error.message : 'Une erreur est survenue')
      };
  ...
  async generateTimeSeriesData(
    ...
    try {
      ...
      logger.error('AiVisualization', 'Error generating time series data:', error);
      return {
        success: false,
        error: (error instanceof Error ? error.message : 'Une erreur est survenue')
      };
