...
export class DataGovernanceService {
  ...
  async searchCompaniesIntelligent(
    ...
    try {
      ...
      logger.error('DataGovernance', '❌ Erreur recherche intelligente:', error);
      throw error;
    }
  }
  /**
  ...
  async getDetectedDuplicates(companyId?: string): Promise<CompanyDuplicate[]> {
    try {
      ...
      logger.error('DataGovernance', '❌ Erreur récupération doublons:', error);
      throw error;
    }
  }
  /**
  ...
  async markAsDuplicate(
    ...
    try {
      ...
      logger.error('DataGovernance', '❌ Erreur marquage doublon:', error);
      return {
        success: false,
  ...
  async mergeCompanies({
    ...
    try {
      ...
      logger.error('DataGovernance', '❌ Erreur fusion entreprises:', error);
      return {
        success: false,
  ...
  async markAsFalsePositive(duplicateId: string): Promise<{ success: boolean }> {
    try {
      ...
      logger.error('DataGovernance', '❌ Erreur marquage faux positif:', error);
  ...
  async analyzeDataQuality(): Promise<DataQualityMetric[]> {
    try {
      ...
      logger.error('DataGovernance', '❌ Erreur analyse qualité:', error);
      throw error;
    }
  }
  /**
  ...
  async getGovernanceStats(): Promise<{
    ...
    try {
      ...
      logger.error('DataGovernance', '❌ Erreur statistiques gouvernance:', error);
      throw error;
    }
  }
  /**
  ...
  async getCompanyAuditTrail(companyId: string, limit: number = 50): Promise<any[]> {
    try {
      ...
      logger.error('DataGovernance', '❌ Erreur audit trail:', error);
      throw error;
    }
  }
  /**
  ...
  async cleanupInactiveData(daysOld: number = 90): Promise<{
    ...
    try {
      ...
      logger.error('DataGovernance', '❌ Erreur nettoyage données:', error);
      throw error;
    }
  }
  /**
  ...
  const arrayData = Array.isArray(data) ? (data as unknown[]) : null;
      ...
      logger.error('DataGovernance', '❌ Erreur recalcul scores:', error);
      throw error;
    }
  }
}
