...
export class CurrencyIntegration {
  ...
  async initializeCurrencySystem(): Promise<void> {
    try {
      logger.warn('CurrencyIntegration', '🏦 Initialisation du système de devises...');
      ...
      logger.warn('CurrencyIntegration', '✅ Système de devises initialisé avec succès');
    } catch (error) {
      logger.error('CurrencyIntegration', '❌ Erreur initialisation devises:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  /**
  ...
  private async createCurrencyTables(): Promise<void> {
    ...
    const queryPromises = sqlQueries.map(async query => {
      try {
        ...
        logger.warn('CurrencyIntegration', 'SQL Query failed (may be normal):', `${query.substring(0, 50)}...`);
  ...
  private async insertSupportedCurrencies(): Promise<void> {
    ...
    const currencyPromises = SUPPORTED_CURRENCIES.map(async currency => {
      try {
        ...
        if (error) {
          logger.warn('CurrencyIntegration', `Erreur insertion devise ${currency.code}:`, error);
        }
      } catch (error) {
        logger.warn('CurrencyIntegration', `Erreur devise ${currency.code}:`, error);
      }
    });
  ...
  private async insertFixedExchangeRates(): Promise<void> {
    ...
    const ratePromises = fixedRates.map(async rate => {
      try {
        ...
        if (error) {
          logger.warn('CurrencyIntegration', `Erreur taux ${rate.from_currency}/${rate.to_currency}:`, error);
        }
      } catch (error) {
        logger.warn('CurrencyIntegration', `Erreur taux fixe:`, error);
      }
    });
  ...
  private async updateExistingTables(): Promise<void> {
    ...
    const updatePromises = updateQueries.map(async query => {
      try {
        const { error } = await supabase.rpc('execute_sql', { sql: query });
        if (error && !error.message.includes('already exists')) {
          logger.warn('CurrencyIntegration', 'Erreur mise à jour table:', error);
        }
      } catch (error) {
        logger.warn('CurrencyIntegration', 'Erreur ALTER TABLE:', error);
      }
    });
  ...
  async migrateCurrencyData(): Promise<void> {
    if (!supabase) {
      logger.warn('CurrencyIntegration', 'Supabase client non disponible, migration ignorée');
      return;
    }
    const config = this.configService.getConfig();
    if (!config?.company?.currency) {
      logger.warn('CurrencyIntegration', 'Aucune devise configurée, migration ignorée');
      return;
    }
    // ✅ CORRECTION: Vérification que config.company.id existe
    if (!(config.company as any).id) {
      logger.warn('CurrencyIntegration', 'ID entreprise manquant, migration ignorée');
      return;
    }
    ...
    try {
      ...
      if (companyError) {
        logger.warn('CurrencyIntegration', 'Erreur mise à jour devise entreprise:', companyError);
      }
      // Mettre à jour les comptes existants
      logger.warn('CurrencyIntegration', 'Mise à jour de la devise des comptes ignorée: chart_of_accounts ne stocke plus de devise par compte.');
      ...
      if (transactionsError) {
        logger.warn('CurrencyIntegration', 'Erreur mise à jour devise transactions:', transactionsError);
      }
      logger.warn('CurrencyIntegration', '✅ Migration des devises terminée');
    } catch (error) {
      logger.error('CurrencyIntegration', '❌ Erreur migration devises:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  /**
  ...
  async validateCurrencySystem(): Promise<{
    ...
    try {
      ...
      logger.error('CurrencyIntegration', '❌ Erreur validation système de devises:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
}
