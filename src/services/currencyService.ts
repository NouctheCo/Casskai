...
export class CurrencyService {
  ...
  async getExchangeRate(fromCurrency: string, toCurrency: string): Promise<number> {
    ...
    try {
      ...
      try {
        ...
        logger.warn('Currency', 'Impossible de sauvegarder en DB:', error);
      }
      return newRate.rate;
    } catch (error) {
      logger.error('Currency', `❌ Impossible de récupérer le taux ${fromCurrency}/${toCurrency}:`, error instanceof Error ? error.message : String(error));
  ...
  async updateExchangeRates(): Promise<void> {
    try {
      logger.warn('Currency', '🔄 Mise à jour des taux de change...');
      for (const provider of this.exchangeProviders.filter(p => p.isActive)) {
        try {
          ...
          logger.warn('Currency', `Erreur avec ${provider.name}:`, error);
          continue;
        }
      }
      this.lastUpdate = new Date();
      logger.warn('Currency', '✅ Taux de change mis à jour');
    } catch (error) {
      logger.error('Currency', '❌ Erreur lors de la mise à jour des taux:', error instanceof Error ? error.message : String(error));
    }
  }
  ...
  private startCriticalRatesUpdater(): void {
    ...
    this.updateCriticalRates().catch(error => 
      logger.warn('Currency', 'Erreur mise à jour initiale taux critiques:', error)
    );
    // Mise à jour toutes les 6 heures
    setInterval(() => {
      this.updateCriticalRates().catch(error =>
        logger.warn('Currency', 'Erreur mise à jour automatique taux critiques:', error)
      );
    }, 6 * 60 * 60 * 1000); // 6 heures
  }
  // Mise à jour spécifique des taux critiques
  async updateCriticalRates(): Promise<void> {
    logger.warn('Currency', '🔄 Mise à jour des taux critiques (EUR/USD/XOF/XAF)...');
    try {
      ...
      logger.warn('Currency', '✅ Taux critiques mis à jour avec succès');
    } catch (error) {
      logger.error('Currency', '❌ Erreur mise à jour taux critiques:', error instanceof Error ? error.message : String(error));
    }
  }
  private async fetchAndCacheCriticalRate(from: string, to: string): Promise<void> {
    try {
      ...
      logger.warn('Currency', `Impossible de récupérer ${from}/${to}:`, error);
    }
  }
  ...
  private async calculateCrossRates(): Promise<void> {
    try {
      ...
      logger.warn('Currency', 'Erreur calcul taux croisés USD:', error);
    }
  }
  private async saveExchangeRateToDB(rate: ExchangeRate): Promise<void> {
    try {
      ...
      if (error) {
        logger.warn('Currency', 'Erreur sauvegarde taux:', error);
      }
    } catch (_error) {
  ...
  async forceCriticalRatesUpdate(): Promise<void> {
    logger.warn('Currency', '🔄 FORCE: Mise à jour des taux critiques...');
