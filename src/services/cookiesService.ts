...
export class CookiesService {
  ...
  static async saveCookiePreferences(preferences: Omit<CookiePreferences, 'id' | 'created_at' | 'updated_at'>): Promise<CookiePreferences> {
    try {
      ...
      logger.error('Cookies', 'Error saving cookie preferences to database:', error instanceof Error ? error.message : String(error));
  ...
  static async getCookiePreferences(): Promise<CookiePreferences | null> {
    try {
      ...
      logger.error('Cookies', 'Error fetching cookie preferences:', error instanceof Error ? error.message : String(error));
      return this.getFromLocalStorage();
    }
  }
  ...
  private static async getClientIP(): Promise<string> {
    try {
      ...
      logger.warn('Cookies', 'Could not fetch client IP:', error);
      return 'unknown';
    }
  }
  ...
  private static getFromLocalStorage(): CookiePreferences | null {
    try {
      ...
      logger.error('Cookies', 'Error reading cookie preferences from localStorage:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  static async getConsentStatistics(): Promise<{
    ...
    try {
      ...
      logger.error('Cookies', 'Error fetching consent statistics:', error instanceof Error ? error.message : String(error));
      return {
        total: 0,
        essential: 0,
        functional: 0,
