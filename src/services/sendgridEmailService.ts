...
export function initializeSendGrid(apiKey: string) {
  if (!apiKey) {
    logger.warn('SendgridEmail', '⚠️  SendGrid API key not provided. Email sending disabled.');
    return false;
  }
  sgMail.setApiKey(apiKey);
  logger.warn('SendgridEmail', '✅ SendGrid initialized');
  return true;
}
/**
...
export async function sendEmail(template: EmailTemplate): Promise<boolean> {
  try {
    ...
    logger.warn('SendgridEmail', `📧 Email sent to ${template.to}`);
    return true;
  } catch (error) {
    logger.error('SendgridEmail', '❌ Email sending failed:', error instanceof Error ? error.message : String(error));
    return false;
  }
}
/**
