...
export class InboundService {
  ...
  async processInboundInvoice(
    ...
    try {
      logger.warn('Inbound', `📥 Processing inbound invoice for company ${companyId}`);
      // Create queue entry
      ...
      logger.warn('Inbound', `✅ Inbound invoice processed successfully: ${creationResult.invoice_id}`);
      return {
        queue_id: queueId,
        status: 'parsed',
      ...
      logger.error('Inbound', 'Error processing inbound invoice:', error);
      if (queueId) {
        await this.updateQueueStatus(
          queueId,
          'error',
  ...
  async getQueueStatus(queueId: string): Promise<EInvInboundQueue | null> {
    try {
      const { data, error } = await supabase
        ...
        .single();
      if (error) {
        logger.error('Inbound', 'Error getting queue status:', error);
        return null;
      }
  ...
  async listInboundInvoices(
    ...
    try {
      ...
      logger.error('Inbound', 'Error listing inbound invoices:', error);
  ...
  async retryProcessing(queueId: string): Promise<{
    ...
    try {
      logger.warn('Inbound', `🔄 Retrying processing for queue item ${queueId}`);
      // Get queue item
      ...
      logger.error('Inbound', 'Error retrying processing:', error);
      if (error instanceof EInvoicingError) {
        throw error;
      }
      throw new EInvoicingError(
  ...
  private async updateQueueStatus(
    ...
    const { error } = await supabase
      ...
      .eq('id', queueId);
    if (error) {
      logger.error('Inbound', 'Error updating queue status:', error);
    }
  }
  private async checkForDuplicates(
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .limit(1);
      if (error) {
        logger.error('Inbound', 'Error checking for duplicates:', error);
        return false; // Don't block processing on duplicate check errors
      }
      return (data && data.length > 0);
    } catch (error) {
      logger.error('Inbound', 'Error in duplicate check:', error);
      return false;
    }
  }
