...
export class InvoiceToEN16931Mapper {
  ...
  async mapInvoiceToEN16931(invoice: CassKaiInvoice): Promise<EN16931Invoice> {
    try {
      ...
      logger.error('InvoiceToEN16931Mapper', 'Error mapping invoice to EN16931:', error);
  ...
  private async calculateTotals(invoice: CassKaiInvoice): Promise<EN16931Totals> {
    ...
    if (Math.abs(totals.invoice_total_with_vat - calculatedTotal) > 0.01) {
      logger.warn('InvoiceToEN16931Mapper', `Total mismatch in invoice ${invoice.invoice_number}: expected ${calculatedTotal}, got ${totals.invoice_total_with_vat}`);
    }
    return totals;
  }
  /**
  ...
  private formatDate(dateString: string): string {
    try {
      ...
      logger.warn('InvoiceToEN16931Mapper', `Error formatting date ${dateString}:`, error);
  ...
  private mapCountryCode(country?: string): CountryCode {
    ...
    logger.warn('InvoiceToEN16931Mapper', `Unknown country: ${country}, defaulting to FR`);
    return 'FR';
  }
