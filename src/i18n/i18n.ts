...
const initConfig = {
  ...
  parseMissingKeyHandler: (key: string) => {
    if (process.env.NODE_ENV === 'development') {
      logger.warn('I18n', `🌍 Parsing missing key: ${key}`);
    }
    return key;
  }
};
...
const initializeI18n = async () => {
  try {
    ...
    if (import.meta.env.DEV && import.meta.env.VITE_DEBUG_MODE === 'true') {
      logger.warn('I18n', 'i18n initialisé avec succès');
    }
    return true;
  } catch (err) {
    logger.error('I18n', 'Erreur lors de l\'initialisation de i18n:', err);
    ...
    try {
      ...
      logger.warn('I18n', 'i18n initialisé avec configuration de secours');
      return true;
    } catch (fallbackErr) {
      logger.error('I18n', 'Erreur d\'initialisation fallback i18n:', fallbackErr);
      return false;
    }
  }
};
...
export async function changeLanguageAndDetectCountry(lng: string) {
  try {
    ...
    if (!i18n || !i18n.changeLanguage) {
      logger.warn('I18n', 'i18n not properly initialized, waiting...');
    ...
    if (i18n && typeof i18n.changeLanguage === 'function') {
      ...
      logger.error('I18n', 'i18n.changeLanguage is not available');
      return false;
    }
  } catch (err) {
    logger.error('I18n', 'Erreur lors du changement de langue:', err);
    return false;
  }
}
...
export function createSafeTranslation(i18nInstance = i18n) {
  return (key: string, fallback?: string, options?: any) => {
    try {
      ...
      logger.warn('I18n', `🌍 Translation error for key '${key}':`, err);
      return fallback || key;
    }
  };
}
...
/**
 * Hook pour obtenir la locale actuelle et les informations associées
 */
export function useLocale() {
  const currentLocale = i18n.language || 'fr';
  const localeInfo = SUPPORTED_LOCALES[currentLocale as keyof typeof SUPPORTED_LOCALES] || SUPPORTED_LOCALES.fr;
  // Fonction de traduction avec fallback
  const t = (key: string, options?: any) => {
    try {
      return i18n.t(key, options) || options?.defaultValue || key;
    } catch {
  ...
  return {
    ...
    changeLanguage: (lng: string) => changeLanguageAndDetectCountry(lng),
