...
export default function PurchasesPage() {
  ...
  const loadPurchasesData = async () => {
    try {
      ...
      logger.error('Purchases', 'Error loading purchases:', error);
  ...
  const loadSuppliers = async () => {
    try {
      ...
      logger.error('Purchases', 'Error loading suppliers:', error);
    }
  };
  // Filter handlers
  ...
  const handleExportCsv = async () => {
    try {
      ...
      logger.error('Purchases', 'Error exporting:', error);
  ...
  const handleGeneratePdf = () => {
    try {
      ...
      logger.error('Purchases', 'Error generating PDF:', error);
    ...
    try {
      ...
      if (!editingPurchase && result.data && currentEnterprise) {
        try {
          ...
          logger.warn('Purchases', 'Auto-accounting generation failed, but purchase was created:', error);
        }
      }
      if (editingPurchase) {
      ...
      logger.error('Purchases', 'Error saving purchase:', error);
  ...
  const handleDeletePurchase = async (id: string) => {
    ...
    if (confirmed) {
      try {
        ...
        logger.error('Purchases', 'Error deleting purchase:', error);
  ...
  const handleMarkAsPaid = async (id: string) => {
    try {
      ...
      logger.error('Purchases', 'Error marking as paid:', error);
      toastError("Erreur lors de l'op\u00e9ration");
    }
  };
  // Animation variants
