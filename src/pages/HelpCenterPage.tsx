...
const HelpCenterPage = () => {
  ...
  const handleSupportOption = (optionName: string) => {
    logger.debug('HelpCenter', `Support option selected: ${optionName}`);

    switch (optionName) {
      case 'Chat en direct':
        // Ouvrir le widget de chat (ex: Intercom, Crisp, etc.)
        // Pour l'instant, rediriger vers le support
        window.open('https://support.casskai.app/chat', '_blank');
        break;
      case 'Support par email':
        // Ouvrir le client email avec adresse pré-remplie
        window.location.href = 'mailto:support@casskai.app?subject=Demande de support CassKai';
        break;
      case 'Support téléphonique':
        // Afficher le numéro ou lancer l'appel
        window.location.href = 'tel:+33123456789';
        break;
      case 'Session de formation':
        // Rediriger vers le calendrier de réservation
        window.open('https://calendly.com/casskai-support/formation', '_blank');
        break;
      default:
        logger.warn('HelpCenter', `Unknown support option: ${optionName}`);
    }
  };
  ...
  const handleDownloadGuides = () => {
    // Télécharger le guide PDF principal
    const link = document.createElement('a');
    link.href = '/docs/CassKai_Guide_Utilisateur.pdf';
    link.download = 'CassKai_Guide_Utilisateur.pdf';
                  ...
                  <CardContent>
                    ...
                    <ul className="space-y-2">
                      {category.topics.slice(0, 3).map((topic, idx) => (
                        <li key={idx} className="text-sm text-gray-600 dark:text-gray-300 hover:text-blue-600 cursor-pointer">
                          • {topic}
                        </li>
                      ))}
                    </ul>
