...
const CookiesPolicyPage = () => {
  ...
  useEffect(() => {
    const loadPreferences = async () => {
      ...
      try {
        ...
        logger.error('CookiesPolicy', 'Error loading cookie preferences:', error instanceof Error ? error.message : String(error));
        toastError("Impossible de charger vos préférences de cookies");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const savePreferences = async () => {
    ...
    try {
      ...
      toastSuccess("Vos préférences de cookies ont été mises à jour avec succès");
    } catch (error) {
      logger.error('CookiesPolicy', 'Error saving cookie preferences:', error instanceof Error ? error.message : String(error));
      toastError("Impossible de sauvegarder vos préférences. Veuillez réessayer.");
    } finally {
      setIsSaving(false);
    }
  };
  const acceptAllCookies = async () => {
    ...
    try {
      ...
      CookiesService.applyCookiePreferences(preferences);
      toastSuccess("Toutes les catégories de cookies ont été activées");
    } catch (error) {
      logger.error('CookiesPolicy', 'Error accepting all cookies:', error instanceof Error ? error.message : String(error));
      toastError("Impossible d'activer tous les cookies. Veuillez réessayer.");
    } finally {
      setIsSaving(false);
    }
  };
  const rejectOptionalCookies = async () => {
    ...
    try {
      ...
      toastSuccess("Seuls les cookies essentiels sont activés");
    } catch (error) {
      logger.error('CookiesPolicy', 'Error rejecting optional cookies:', error instanceof Error ? error.message : String(error));
      toastError("Impossible de refuser les cookies optionnels");
    } finally {
      setIsSaving(false);
    }
  };
