...
const CeoSection: React.FC<{
  ...
  <Select value={companyData.ceoTitle ?? 'CEO'} onValueChange={(value) => updateField('ceoTitle', value)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="Présidente">Présidente</SelectItem>

            <SelectItem value="Directeur Général">Directeur Général</SelectItem>

            <SelectItem value="Directrice Générale">Directrice Générale</SelectItem>

            <SelectItem value="Gérant">Gérant</SelectItem>

            <SelectItem value="Gérante">Gérante</SelectItem>
            ...
            <SelectItem value="Associée">Associée</SelectItem>

            <SelectItem value="Administrateur">Administrateur</SelectItem>

            <SelectItem value="Administratrice">Administratrice</SelectItem>
...
const FiscalYearSection: React.FC<{
  ...
  const calculateFiscalPeriod = (startMonth: number, startDay: number) => {
    ...
    const normalizedMonth = ((isNaN(startMonth) ? 0 : startMonth) + 12) % 12;

    const normalizedDay = Math.min(Math.max(startDay || 1, 1), 31);
    ...
    if (normalizedDay === 1) {
      ...
      const prevMonth = normalizedMonth === 0 ? 11 : normalizedMonth - 1;

      const year = new Date().getFullYear();

      const lastDay = new Date(year, prevMonth + 1, 0).getDate();

      return `Du 1er ${months[normalizedMonth]} au ${lastDay} ${months[prevMonth]} (année suivante)`;

    }



    const endDay = normalizedDay - 1;

    return `Du ${normalizedDay} ${months[normalizedMonth]} au ${endDay} ${months[endMonth]} (année suivante)`;

  };



  const normalizedStartMonth = Math.max(1, companyData.fiscalYearStartMonth ?? 1) - 1;

  const startDay = companyData.fiscalYearStartDay ?? 1;

  const fiscalPeriod = calculateFiscalPeriod(normalizedStartMonth, startDay);
        ...
        <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 space-y-4">
          ...
          <div className="bg-indigo-100 border border-indigo-300 rounded-lg p-3">
            ...
            <p className="text-indigo-800 text-sm">
              ...
              <strong>Exercice fiscal :</strong> {fiscalPeriod}
        ...
        <CardContent className="p-6 space-y-6">
          ...
          {referentialsLoading ? (
            ...
            <div className="flex items-center justify-center py-8">
              ...
              <span className="text-gray-600 dark:text-gray-300">Chargement des référentiels...</span>
