...
const UserManagementPage = () => {
  ...
  const handleInviteSubmit = async (e: React.FormEvent) => {
    ...
    if (!foundRole || !foundInvitedBy) {
      logger.error('UserManagement', 'Role or inviter not found');
      return;
    }
  ...
  const StatusBadge = ({ status }: { status: 'active' | 'inactive' | 'pending' | 'suspended' | string }) => {
  ...
  const RoleBadge = ({ role }: { role: Role }) => {
    ...
    const Icon = roleIcons[roleLevel] || UserIcon;
    ...
    return (
      <Badge className={`${colors[roleLevel]} flex items-center gap-1`}>
        <Icon className="h-3 w-3" />
        {role.name}
      </Badge>
    );
