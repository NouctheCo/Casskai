...
export const ModulesProvider: React.FC<ModulesProviderProps> = ({
  ...
  const registerAvailableModules = async () => {
    try {
      ...
      if (!isE2EMinimal) {
        // FIX: Importer seulement les modules qui existent réellement
        try {
          // Seul le module CRM existe pour le moment
          const crmPath = '@/modules/crm/crmModule';
          importsPromises.push(import(/* @vite-ignore */ crmPath));
          
          console.log('[ModulesProvider] Chargement du module CRM uniquement');
        } catch (error) {
          console.error('[ModulesProvider] Erreur lors du chargement des modules:', error);
        }
      }
      ...
      // FIX: Enregistrer seulement le module CRM pour l'instant
      moduleImports.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          // Seul le module CRM est chargé
          const moduleExport = (result.value as any).crmModule;
          
          if (moduleExport) {
            try {
              moduleManager.registerModule(moduleExport);
              console.log(`[ModulesProvider] Module ${moduleExport.definition.id} enregistré avec succès`);
            } catch (error) {
              console.error(`[ModulesProvider] Erreur enregistrement module CRM:`, error);
            }
          } else {
            console.warn(`[ModulesProvider] Export crmModule introuvable dans le module importé`);
          }
        } else {
