    ...
    const initializeLanguage = async () => {
      try {
        ...
        while ((!i18n.isInitialized || !i18n.hasResourceBundle) && attempts < 50) {
          logger.warn('Locale', 'i18n not ready, waiting... attempt', attempts + 1);
          await new Promise(resolve => setTimeout(resolve, 100));
          attempts++;
        }
        if (!i18n.isInitialized) {
          logger.error('Locale', 'i18n failed to initialize after 5 seconds');
        ...
        logger.debug('Locale', '✅ Langue initialisée:', locale);
      } catch (error) {
        logger.error('Locale', 'Error initializing language:', error instanceof Error ? error.message : String(error));
        setLoadingLocale(false);
      }
    };
    initializeLanguage();
    ...
    try {
      ...
      if (translation && typeof translation === 'string') {
        ...
        if (translation.includes("key '") && translation.includes("' return")) {
          logger.warn('Locale', `Translation key '${key}' not found, using fallback: ${currentDefaultValue}`);
      ...
      logger.error('Locale', `Error translating key '${key}':`, error instanceof Error ? error.message : String(error));
      return currentDefaultValue;
    }
  };
