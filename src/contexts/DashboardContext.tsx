...
export function DashboardProvider({ children }: { children: ReactNode }) {
  ...
  const initializeDefaultDashboard = async () => {
    try {
      ...
      logger.error('Dashboard', 'Error initializing default dashboard:', error);
  ...
  const loadDashboards = async () => {
    try {
      ...
      logger.error('Dashboard', 'Error loading dashboards:', error);
  ...
  const updateDashboard = async (updates: Partial<DashboardConfig>) => {
    ...
    try {
      ...
      logger.error('Dashboard', 'Error updating dashboard:', error);
  ...
  const deleteDashboard = async (id: string) => {
    try {
      ...
      logger.error('Dashboard', 'Error deleting dashboard:', error);
  ...
  const setCurrentDashboard = async (id: string) => {
    try {
      ...
      logger.error('Dashboard', 'Error setting current dashboard:', error);
  ...
  const addWidget = async (widget: Omit<WidgetConfig, 'id'>): Promise<string> => {
    ...
    try {
      ...
      logger.error('Dashboard', 'Error adding widget:', error);
  ...
  const updateWidget = async (id: string, updates: Partial<WidgetConfig>) => {
    try {
      ...
      logger.error('Dashboard', 'Error updating widget:', error);
  ...
  const removeWidget = async (id: string) => {
    try {
      ...
      logger.error('Dashboard', 'Error removing widget:', error);
  ...
  const updateLayout = async (layout: WidgetLayout[]) => {
    ...
    (updateLayout as typeof updateLayout & { timeoutId?: NodeJS.Timeout }).timeoutId = setTimeout(async () => {
      try {
        ...
        logger.error('Dashboard', 'Error updating layout:', error);
      }
    }, 1000);
  };
  const loadTemplates = async () => {
    try {
      ...
      logger.error('Dashboard', 'Error loading templates:', error);
    }
  };
  const applyTemplate = async (templateId: string) => {
    ...
    try {
      ...
      logger.error('Dashboard', 'Error applying template:', error);
  ...
  const saveAsTemplate = async (template: Omit<DashboardTemplate, 'id'>): Promise<string> => {
    try {
      ...
      logger.error('Dashboard', 'Error saving template:', error);
    ...
    realtimeChannel.current = supabase
      ...
      .on('presence', { event: 'sync' }, () => {
        ...
        const collaborators = Object.entries(presenceState).map(([userId, presence]: [string, unknown[]]) => ({
          ...
          color: (presence[0] as Record<string, unknown>)?.color as string || '#3B82F6'
        }));
        dispatch({ type: 'SET_COLLABORATORS', payload: collaborators as any });
      })
  ...
  const loadDashboardFromStorage = () => {
    try {
      ...
      logger.error('Dashboard', 'Error loading from storage:', error);
    }
  };
  ...
  const importDashboard = async (data: string) => {
    try {
      ...
      logger.error('Dashboard', 'Error importing dashboard:', error);
