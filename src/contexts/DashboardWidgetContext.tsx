...
export function DashboardWidgetProvider({ children }: { children: React.ReactNode }) {
  ...
  const saveLayout = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        logger.error('DashboardWidget', 'Error saving layout:', error);
      }
    } catch (error) {
      logger.error('DashboardWidget', 'Error saving layout:', error instanceof Error ? error.message : String(error));
    }
  }, [currentLayout, user]);
