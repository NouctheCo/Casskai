...
export function useHR(): UseHRReturn {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      try {
        ...
        console.error('[useHR] Auth check failed:', err);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      console.log('[useHR] Auth state changed:', event, !!session);
  ...
  useEffect(() => {
    ...
    if (currentCompany?.id && isAuthReady) {
      ...
      console.log('[useHR] Loading initial data for company:', currentCompany.id);
      ...
      console.warn('[useHR] Waiting for auth to be ready before loading data');
